...
export class UsersService {
    ...
    public login(userLoginModel: UserLoginModel): void {
        this.client.post<ApiResponse<string>>(`${baseUrl}/users/login`, userLoginModel)
            .subscribe(res => {
                this.localService.saveData(TOKEN_NAME, res.result);
            });
    }

    public register(userRegisterModel: UserRegisterModel): void {
        this.client.post<ApiResponse<string>>(`${baseUrl}/users/register`, userRegisterModel)
            .subscribe(res => {
                this.localService.saveData(TOKEN_NAME, res.result);
            });
    }
    ...
    public getCurrentUser(): Observable<ApiResponse<UserModel>> {
       return this.client.post<ApiResponse<UserModel>>(`${baseUrl}/users/getcurrentuser`, null);
    }

    public isLoggedIn(): boolean {
        return this.localService.getData(TOKEN_NAME) !== null;
    }
}